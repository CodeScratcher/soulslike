...
func _physics_process(delta):
	if stamina <= 0 and state != PlayerState.ATTACK and state != PlayerState.ROLL:
		...
		MAX_SPEED = 150.0
	if not ($AnimationPlayer.current_animation == "attack" or $AnimationPlayer.current_animation == "heavy_attack" or $AnimationPlayer.current_animation == "roll" or $AnimationPlayer.current_animation == "roll_left" or $BlockArea.visible) and is_on_floor() and not state == PlayerState.RECOVER:
